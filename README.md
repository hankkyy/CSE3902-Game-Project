...
| --------------- | ---------------------------------- |
| **Todo**        | Task has not been started(任务尚未开始)          |
| **In Progress** | Someone is currently working on it(有人正在处理中) |
| **Done**        | Task is completed(任务已完成)                  |
...
**Task:** Player Movement(**任务:** 玩家移动)
**Assignee:** `your-github-username`(**负责人:** `your-github-username`)
**Status:** Todo(**状态:** Todo)
**Effort:** 3(**工作量:** 3)
...
```csharp
// Sprint 1 GitHub practice(Sprint 1 GitHub 练习)
```
...
```text
Add player movement(添加玩家移动)
Fix enemy collision(修复敌人碰撞)
Add room transition logic(添加房间切换逻辑)
Update player sprite(更新玩家精灵)
```
